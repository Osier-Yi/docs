...
| DashScope (Realtime) | `DashScopeRealtimeTTSModel` | WebSocket 流式输入，适合对接 LLM 流式输出 |
...
实时模型支持增量推送文本（典型场景：对接 LLM 的流式输出）。通过 `async with` 或手动调用 `connect()` / `close()` 管理生命周期：
