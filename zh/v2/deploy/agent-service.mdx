---
title: "Agent Service"
description: "把 agent 部署为多租户、多会话的 HTTP 服务"
---

Agent Service 是基于 FastAPI 的托管层，把 AgentScope 的 agent 转化为**多租户、多会话的 HTTP 服务**。它接管 agent *外围*的全部职责 —— 请求路由、按用户的资源生命周期、会话状态、持久化、调度，以及工具调用的卸载，让基于 [`Agent`](/v2/building-blocks/agent) 编写的代码无需重写即可承接生产流量。
...
Agent Service 自身不建模用户系统。`user_id` 是通过 `get_current_user_id` 依赖解析出的不透明租户标识 —— 替换该依赖即可对接任何身份系统（JWT、OAuth2、session cookie、SSO、API token）。详见[扩展鉴权](#extend-authentication)。
...
<ParamField path="workspace_manager" type="WorkspaceManagerBase | None" default="None">
  以 TTL 缓存方式管理 workspace（文件存储、MCP server、skill）。内置 `LocalWorkspaceManager` 按 agent 隔离；其他策略见 [Workspace 实现与隔离](#workspace-implementation-and-isolation)。
</ParamField>
...
<Warning>
默认的 `X-User-ID` header 不提供任何鉴权。生产部署前请替换为真实的鉴权方案 —— 见[用户鉴权](#user-authentication)。
</Warning>
...
Chat 端点通过 SSE 输出 AgentScope 原生的 [`AgentEvent`](/v2/building-blocks/message-and-event) 流。要让同一 agent 服务于不同前端协议，安装协议 middleware 拦截 SSE 流并改写每帧。
