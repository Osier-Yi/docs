...
<Tip>
嵌入模型的介绍和配置方式请见[嵌入模型章节](/versions/2.0.5dev/zh/building-blocks/model)；服务化版本的 RAG（带 HTTP 服务、文件托管、分布式索引）请见 [RAG 服务](/versions/2.0.5dev/zh/deploy/rag)。
</Tip>
...
请见[嵌入模型章节](/versions/2.0.5dev/zh/building-blocks/model)。
...
<CardGroup cols={1}>
  ...
  <Card title="嵌入模型" icon="cube" href="/versions/2.0.5dev/zh/building-blocks/model" cta="查看模型列表" arrow>
    可用的嵌入模型及其参数。
  </Card>
</CardGroup>
