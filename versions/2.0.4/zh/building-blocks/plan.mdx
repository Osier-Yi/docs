...
- [Tool](/versions/2.0.4/zh/building-blocks/tool) —— toolkit、`ToolBase` 接口，以及状态注入式工具如何拿到 `AgentState`。
- [Agent](/versions/2.0.4/zh/building-blocks/agent) —— 智能体生命周期，包括 `AgentState` 的创建、恢复与持久化。
