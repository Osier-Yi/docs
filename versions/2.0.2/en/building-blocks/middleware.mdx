...
`TTSMiddleware` intercepts the agent's text output and synthesizes speech audio, injecting `DATA_BLOCK_*` events into the event stream alongside the text. It hooks into `on_reply` to observe every `TextBlockDeltaEvent` and `TextBlockEndEvent`.
