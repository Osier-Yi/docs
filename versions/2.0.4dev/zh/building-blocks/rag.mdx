...
<Tip>
嵌入模型的介绍和配置方式请见[嵌入模型章节](/versions/2.0.4dev/zh/building-blocks/model)；服务化版本的 RAG（带 HTTP 服务、文件托管、分布式索引）请见 [RAG 服务](/versions/2.0.4dev/zh/deploy/rag)。
</Tip>
...
请见[嵌入模型章节](/versions/2.0.4dev/zh/building-blocks/model)。
...
<CardGroup cols={1}>
  <Card title="RAG 服务" icon="server" href="/versions/2.0.4dev/zh/deploy/rag" cta="查看部署文档" arrow>
