...
- **`stream=True`** —— 返回 `AsyncGenerator[ChatResponse, None]`。中间 chunk（`is_last=False`）只携带**增量**内容；最后一个 chunk（`is_last=True`）承载**完整的累积内容**。如果提供商的 `_call_api` 只产出增量 chunk，`ChatModelBase.__call__` 会自动累积并追加最终 chunk，因此调用方和自定义模型实现都无需自行累积增量。
...
<Info>
对自定义流式提供商而言，直接以 `ChatResponse(content=..., is_last=False)` 产出提供商增量即可。如果 `_call_api` 结束时没有产出 `is_last=True` 的最终 chunk，`ChatModelBase.__call__` 会通过 `ChatResponse.append_chat_response()` 自动累积这些增量并发出最终响应；如果流式过程被取消，则会发出带有部分累积内容且 `finished_reason=FinishedReason.INTERRUPTED` 的响应。
</Info>
...
在 agent 层，TTS 通过 [`TTSMiddleware`](/versions/2.0.4dev/zh/building-blocks/middleware#ttsmiddleware) 集成 —— 自动拦截 agent 的文本输出并合成语音：
