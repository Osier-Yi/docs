---
title: "发布说明"
description: "AgentScope 2.x 版本历史，每个版本按模块分类列出新增、变更与修复内容。"
---

本页面记录 AgentScope 2.x 各版本的重要变更。每个版本都被组织为三个部分 —— **新增**、**变更** 与 **修复** —— 同时每个部分内部按模块（如 *Model*、*Tool*、*WebUI* 等）进一步归类，方便你快速浏览自己关心的领域的改动。
...
- **Task 工具**：新增 `TaskCreate`、`TaskGet`、`TaskList`、`TaskUpdate`（详见 [Plan](/versions/2.0.3dev/zh/building-blocks/plan) 页面）。([#1549](https://github.com/agentscope-ai/agentscope/pull/1549))
