...
Agent 的短期记忆存放在[上下文](/versions/2.0.3dev/zh/building-blocks/context)中，会话结束即消失。**长期记忆**让 agent 能够留存持久的信息 —— 用户偏好、过往决策、累积的知识 —— 并在后续会话中重新调取。

在 AgentScope 中，长期记忆以[中间件](/versions/2.0.3dev/zh/building-blocks/middleware)的形式实现。记忆后端不是绑死在 agent 类上的存储，而是一个 `MiddlewareBase` 子类，可以按需挂接到 agent 生命周期的任意 hook 位置。例如：
...
`Mem0Middleware` 位于一个可选的 extra 中：
