...
在 AgentScope 中，长期记忆以[中间件](/versions/2.0.3dev/zh/building-blocks/middleware)的形式实现。**记忆后端**指对接具体记忆提取和存储服务（如 mem0）的中间件实现，例如 `Mem0Middleware`。每个记忆后端是一个 `MiddlewareBase` 子类，可以按需挂接到 agent 生命周期的 hook 位置。例如：
