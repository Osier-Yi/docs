---
title: "Plan"
description: "为 agent 提供结构化任务清单，用于规划、追踪并协调复杂工作"
---
...
- [Tool](/versions/2.0.3dev/zh/building-blocks/tool) —— toolkit、`ToolBase` 接口，以及状态注入式工具如何拿到 `AgentState`。
- [Agent](/versions/2.0.3dev/zh/building-blocks/agent) —— agent 生命周期，包括 `AgentState` 的创建、恢复与持久化。
