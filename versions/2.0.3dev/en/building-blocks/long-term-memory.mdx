...
In AgentScope, long-term memory is implemented as [middleware](/versions/2.0.3dev/en/building-blocks/middleware). A **memory backend** is a middleware implementation that connects to a specific memory extraction and storage service (such as mem0) — for example, `Mem0Middleware`. Each backend is a `MiddlewareBase` subclass that can hook into any position of the agent lifecycle as needed. For example:
