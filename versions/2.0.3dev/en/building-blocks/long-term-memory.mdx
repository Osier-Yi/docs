...
An agent's short-term memory lives in [context](/versions/2.0.3dev/en/building-blocks/context) and disappears when the session ends. **Long-term memory** lets an agent retain durable facts — user preferences, prior decisions, accumulated knowledge — and recall them in later sessions.

In AgentScope, long-term memory is implemented as [middleware](/versions/2.0.3dev/en/building-blocks/middleware). Rather than bolting a memory store onto the agent class, a memory backend is a `MiddlewareBase` subclass that can hook into any position of the agent lifecycle as needed. For example:
...
`Mem0Middleware` lives behind an optional extra:
