...
See [Plan](/versions/2.0.3dev/en/building-blocks/plan) for the full task lifecycle, the storage model, and how to seed or customize tasks programmatically.
...
- **`PermissionDecision(..., bypass_immune=True)`** — set on a returned ASK to mark it as a safety check that allow rules cannot silence (e.g. a `DeployTool` flagging `prod-*` targets). See the [safety check contract](/versions/2.0.3dev/en/building-blocks/permission-system#safety-check-contract) for per-mode handling.
</Note>
...
This pattern underlies the [human-in-the-loop](/versions/2.0.3dev/en/building-blocks/agent) workflow, where certain actions require human approval or manual execution.
...
<CardGroup cols={2}>
  ...
  <Card title="Permission System" icon="shield" href="/versions/2.0.3dev/en/building-blocks/permission-system">
    Fine-grained control over which tools can execute and when
  </Card>
  <Card title="Middleware" icon="layer-group" href="/versions/2.0.3dev/en/building-blocks/middleware">
    Intercept agent lifecycle hooks — reply, reasoning, model calls, and more
  </Card>
  <Card title="Human-in-the-Loop" icon="user" href="/versions/2.0.3dev/en/building-blocks/agent#human-in-the-loop">
