...
Agent Service models no user system of its own. `user_id` is an opaque tenant identifier resolved through the `get_current_user_id` dependency — replace it with a one-function adapter to whatever identity system you use (JWT, OAuth2, session cookies, SSO, API tokens). See [Extend Authentication](#extend-authentication).
...
import uvicorn
from agentscope.app import create_app, RedisStorage, DockerWorkspaceManager
...
```python E2B
import uvicorn
from agentscope.app import create_app, RedisStorage, E2BWorkspaceManager
...
<Steps>
  ...
  <Step title="Create and configure a credential">
    Discover each provider's form fields with `GET /credentials/schemas`, then save the API key. One credential can be reused across many sessions and agents.

    ```http
    GET  /credentials/schemas
    POST /credentials
    ```
  </Step>
...
| Agents | `GET/POST/PATCH/DELETE /agents` | Manage agent records — display name, system prompt, runtime config. |
| Credentials | `GET/POST/PATCH/DELETE /credentials` | CRUD for per-provider API keys and connection configs. |
| Credential schemas | `GET /credentials/schemas` | Discover all registered credential types and their JSON parameter schemas for form rendering. |
| Models | `GET /credentials/{id}/models` | List available models for a credential, with their declarative `ModelCard` (capabilities and parameter schemas). |
| Schedules | `GET/POST/PATCH/DELETE /schedules` | Manage cron-based agent execution, stateful or stateless. |
| Background tasks | `GET /background-tasks` | Inspect offloaded tool executions. |
| Workspace MCPs | `GET/POST /workspace/mcp`, `DELETE /workspace/mcp/{name}` | Manage MCP clients attached to the session's workspace. |
| Workspace skills | `GET/POST /workspace/skill`, `DELETE /workspace/skill/{name}` | Manage skills available in the session's workspace. |
...
```python
from agentscope.app import get_current_user_id as default_dependency
...
The service automatically exposes the credential's JSON schema under `GET /credentials/schemas` and routes `GET /credentials/{id}/models` to the chat model class returned by `get_chat_model_class()`.

### Provider models

The model list returned by `GET /credentials/{id}/models` is built from `ModelCard` instances — declarative metadata records that tell the frontend how to display each model and what request parameters are valid. Each chat model exposes its catalog through `list_models()`, which by default loads `ModelCard` entries from YAML files in the provider's model directory; `ModelCard.from_yaml()` parses each YAML and merges its overrides into the base parameter schema supplied by the chat model's parameters class.
...
| `parameter_overrides` | Per-model deltas applied on top of the base parameter schema. |
...
To add a new model under an existing provider, drop a YAML file alongside the others in the provider's model directory — the loader picks it up automatically and the new entry shows up in `GET /credentials/{id}/models`.
...
```python
from agentscope.app import StorageBase
