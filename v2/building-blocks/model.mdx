...
| DashScope (Realtime) | `DashScopeRealtimeTTSModel` | WebSocket streaming input, ideal for LLM output piping |
...
```python Realtime (Streaming Input)
import os
...
For realtime models, text can be pushed incrementally as it arrives from a streaming LLM. The lifecycle is managed via `async with` or manual `connect()` / `close()`:
