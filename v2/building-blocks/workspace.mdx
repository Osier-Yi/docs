...
- **Workspace Layer** — unified interface (`WorkspaceBase`) with two branches:
  - `LocalWorkspace` — inherits directly from `WorkspaceBase`
  - `WorkspaceWithMCP(WorkspaceBase)` — intermediate base that adds in-container MCP gateway management; `DockerWorkspace` and `E2BWorkspace` inherit from it
...
<ParamField path="skill_paths" type="list[str] | None" default="None">
  Local directories to seed as skills on first initialization. Each must contain a `SKILL.md` with `name` and `description` fields in its YAML front matter.
</ParamField>
...
1. If `.mcp` exists — restore MCPs from the file (ignoring `default_mcps`), reconnect stateful clients
...
`DockerWorkspace` runs an isolated Docker container with an in-container MCP Gateway that aggregates tools from multiple MCP servers into a single REST endpoint. It inherits from `WorkspaceWithMCP`.
...
<ParamField path="instructions" type="str" default="built-in">
  Custom system prompt fragment.
</ParamField>

### MCP Gateway

The gateway is a lightweight Python process running inside the container. It connects to all configured MCP servers and exposes a unified REST API to the host. Gateway dependencies (`mcp`, `uvicorn`, `starlette`) are automatically installed if missing.
...
On the host side, `_RestGatewayClient` (an `MCPClient` subclass) and `_RestMCPTool` (a `ToolBase` adapter) transparently proxy tool calls over HTTP, making gateway-backed tools behave identically to direct MCP tools from the agent's perspective.

## Using E2BWorkspace

`E2BWorkspace` uses [E2B](https://e2b.dev/) cloud sandboxes for fully isolated, ephemeral environments. The architecture is identical to `DockerWorkspace` (in-sandbox MCP gateway via `WorkspaceWithMCP`), but uses the E2B SDK for provisioning.
...
<ParamField path="domain" type="str" default="">
  Optional E2B API domain override.
</ParamField>
...
<ParamField path="gateway_port" type="int" default="5600">
  Port the in-sandbox MCP gateway listens on.
</ParamField>
<ParamField path="env" type="dict[str, str] | None" default="None">
...
1. **Directory scanning** — `list_skills()` reads all subdirectories concurrently, loads each `SKILL.md`, and returns valid skills
...
3. **Concurrency safety** — `add_skill()` and `remove_skill()` are serialized by an internal `_skill_lock`
4. **Manual changes** — skills can be manually added or removed from the `skills/` directory; changes are picked up automatically on the next `list_skills()` call

**DockerWorkspace** deploys skills by copying the directory into the container via tar archive (`container.put_archive`), and discovers skills via `find` command + `SKILL.md` parsing inside the container.

**E2BWorkspace** deploys skills by writing individual files via the E2B SDK (`sandbox.files.write`), and discovers skills the same way as Docker (shell `find` + parsing).
...
Messages containing base64-encoded binary data (images, files) are automatically decoded and saved to the `data/` directory during offloading. The base64 content is replaced with a `file://` URL reference, reducing serialization overhead. Files are content-addressed (SHA-256 hash as filename) to avoid duplicates.
...
sequenceDiagram
    ...
    Service->>WM: get_workspace(user_id, agent_id, session_id, workspace_id)
    WM-->>Service: workspace (from memory, or create on cache miss)
...
`LocalWorkspaceManager` overrides `get_workspace` to perform TTL eviction before lookup and refresh the last-access timestamp on cache hit.
...
<ParamField path="default_mcp_servers" type="list[MCPServerConfig] | None" default="None">
  MCP servers configured in every new workspace.
</ParamField>
<ParamField path="gateway_port" type="int" default="5600">
  Default gateway port for new workspaces.
</ParamField>
<ParamField path="default_env" type="dict[str, str] | None" default="None">
  Environment variables set in every new container.
</ParamField>
<ParamField path="default_startup_commands" type="list[str] | None" default="None">
  Shell commands run in every new container after creation.
</ParamField>
...
<ParamField path="api_key" type="str" default="">
  E2B API key (shared across all workspaces).
</ParamField>
<ParamField path="domain" type="str" default="">
  Optional E2B API domain override.
</ParamField>
<ParamField path="timeout_seconds" type="int" default="300">
  Default sandbox auto-shutdown timeout.
</ParamField>
<ParamField path="working_dir" type="str" default="/home/user">
  Default working directory inside sandboxes.
</ParamField>
<ParamField path="default_mcp_servers" type="list[MCPServerConfig] | None" default="None">
  MCP servers configured in every new workspace.
</ParamField>
...
<ParamField path="default_env" type="dict[str, str] | None" default="None">
  Environment variables set in every new sandbox.
</ParamField>
<ParamField path="default_metadata" type="dict[str, str] | None" default="None">
  Metadata attached to every new sandbox.
</ParamField>
<ParamField path="default_startup_commands" type="list[str] | None" default="None">
  Shell commands run in every new sandbox after creation.
</ParamField>
...
| `get_workspace(user_id, agent_id, session_id, workspace_id)` | Look up a live workspace; creates on cache miss |
