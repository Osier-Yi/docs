...
│   ├── .skills                 # index file (hash + agent-facing name)
│   └── {skill_name}/
│       └── SKILL.md
└── sessions/
    └── {session_id}/
...
### How Skill Indexing Works (LocalWorkspace)

`LocalWorkspace` maintains a `.skills` index file inside the `skills/` directory:

1. **Content-hash deduplication** — each skill is identified by a SHA-256 hash of its `SKILL.md` content; duplicate installations are silently skipped
2. **Agent-facing name resolution** — if a name collision occurs, the workspace appends a counter (e.g., "Web Search (1)")
3. **Filesystem reconciliation** — on `list_skills()`, the workspace compares the index with actual subdirectories. Manually added directories are auto-indexed; deleted directories are pruned from the index
